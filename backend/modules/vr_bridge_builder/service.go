...
var MaterialPresets = map[string]*models.MasonryMaterial{
	"ancient_stone": {
		MaterialName:            "青灰砂岩砌体",
...
var ParamLimits = map[string]map[string][2]float64{
	"ancient_stone": {
		"span_m":        {5, 120},
		"rise_m":        {1, 40},
		"thickness_m":   {0.4, 4.0},
...
func (vb *VRBridgeBuilder) DesignAndTest(design models.VirtualBridgeDesign) (*models.VirtualBridgeResult, error) {
	mat, ok := MaterialPresets[design.MaterialPreset]
	if !ok {
		return nil, fmt.Errorf("MaterialPreset must be one of ancient_stone, modern_rc, steel, got %s", design.MaterialPreset)
	}
	limits, _ := ParamLimits[design.MaterialPreset]
	...
	femResultCh := fem.AsyncRunFullAnalysis(liveLoadPa, 0)
	femResult := <-femResultCh
	if femResult.Error != nil {
		return nil, fmt.Errorf("FEM analysis failed: %v", femResult.Error)
	}
	stresses := femResult.Stresses
	fem.Nodes = femResult.Nodes
	fem.Elements = femResult.Elements
...
func (vb *VRBridgeBuilder) DesignAndTestAsync(design models.VirtualBridgeDesign) <-chan *models.VirtualBridgeResult {
	resultCh := make(chan *models.VirtualBridgeResult, 1)
	vb.WorkerPool.Submit(func() {
		defer close(resultCh)
		result, err := vb.DesignAndTest(design)
		if err != nil {
			resultCh <- &models.VirtualBridgeResult{
				Report: &models.BridgeDesignReport{Recommendation: fmt.Sprintf("计算失败: %v", err)},
			}
			return
		}
		resultCh <- result
