...
func TestDesignAndTest_ZhaozhouStyle(t *testing.T) {
	vb := NewVRBridgeBuilder(newTestWorkerPool())
...
func TestDesignAndTest_InvalidLiveLoad(t *testing.T) {
	...
	testCases := []struct {
		...
		{"负数_stone", -10, "ancient_stone"},
		{"过大_stone", 50.1, "ancient_stone"},
		{"过大_rc", 100.1, "modern_rc"},
		{"过大_steel", 200.1, "steel"},
	}
...
func TestDesignAndTest_BoundaryLiveLoad(t *testing.T) {
	...
	testCases := []struct {
		...
		{"下限0kPa_stone", 0, "ancient_stone"},
		{"上限50kPa_stone", 50, "ancient_stone"},
		{"上限100kPa_rc", 100, "modern_rc"},
		{"上限200kPa_steel", 200, "steel"},
	}
...
func TestDesignAndTest_CreativeDesigns(t *testing.T) {
	...
	for _, tc := range designs {
		t.Run(tc.name, func(t *testing.T) {
			result, err := vb.DesignAndTest(tc.design)
			if err != nil {
...
func TestDesignAndTestAsync_Basic(t *testing.T) {
	vb := NewVRBridgeBuilder(newTestWorkerPool())
	...
	resultCh := vb.DesignAndTestAsync(design)
	result := <-resultCh
	...
	if result.Report.Recommendation != "" && result.Design == nil {
		t.Error("成功结果应有Design数据")
	}
}

func TestDesignAndTestAsync_InvalidParams(t *testing.T) {
	vb := NewVRBridgeBuilder(newTestWorkerPool())
	...
	if !strings.Contains(result.Report.Recommendation, "计算失败") {
		t.Error("无效参数应返回计算失败信息")
	}
}

func TestMaterialPresets_HaveSource(t *testing.T) {
	for name, mat := range MaterialPresets {
		if mat.Source == "" {
			t.Errorf("材料%s缺少文献来源", name)
		}
		if mat.Grade == "" {
			t.Errorf("材料%s缺少等级标注", name)
		}
	}
}

func TestParamLimits_ValidRanges(t *testing.T) {
	for material, limits := range ParamLimits {
		for param, rng := range limits {
			if rng[0] >= rng[1] {
				t.Errorf("材料%s的参数%s范围无效: [%.0f, %.0f]", material, param, rng[0], rng[1])
			}
			if rng[0] < 0 {
				t.Errorf("材料%s的参数%s下限为负: %.0f", material, param, rng[0])
			}
		}
	}
}

func TestNewVRBridgeBuilder_NilPool(t *testing.T) {
	vb := NewVRBridgeBuilder(nil)
	if vb == nil {
		t.Fatal("NewVRBridgeBuilder(nil)不应返回nil")
	}
	if vb.WorkerPool == nil {
...
func TestDesignAndTest_RiseSpanRatioCheck(t *testing.T) {
	vb := NewVRBridgeBuilder(newTestWorkerPool())
	...
	if !strings.Contains(result.Report.DesignNotes, "矢跨比") {
		t.Error("矢跨比偏小时应有设计说明")
	}
	if result.Report.RationalDesign {
		t.Error("矢跨比1/15超出合理范围，RationalDesign应为false")
	}
}

func TestDesignAndTest_SteelWithSmallArches(t *testing.T) {
	vb := NewVRBridgeBuilder(newTestWorkerPool())
	...
	if !strings.Contains(result.Report.DesignNotes, "钢拱桥通常不设敞肩小拱") {
		t.Error("钢拱桥设小拱应有设计说明")
	}
}
